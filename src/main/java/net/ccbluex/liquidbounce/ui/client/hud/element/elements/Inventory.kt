    ...
    override fun drawElement(): Border {
        ...
        for (i in 0 until gradientAmountValue.get()) {
            ...
            RenderUtils.drawGradientSideways(8+ barStart, 28.0, 8 + barEnd, 29.0,
                when (rainbowType) {
                    "CRainbow" -> RenderUtils.getRainbowOpaque(cRainbowSecValue.get(), saturationValue, brightnessValue, (i + 1) * distanceValue.get())
                    "Sky" -> RenderUtils.SkyRainbow((i + 1) * distanceValue.get(), saturationValue, brightnessValue)
                    "LiquidSlowly" -> ColorUtils.LiquidSlowly(System.nanoTime(), (i + 1) * distanceValue.get(), saturationValue, brightnessValue).rgb
